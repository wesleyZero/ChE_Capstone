		...
		for P_i = P_RANGE
			for MR_S_i = STEAM_RANGE
				...
				for i = 2:length(F_soln_ODE(:, 1))
					...
					% F_steam = MR_MOL_2_MASS_CONV_FACTOR * MR_S_i * (F_fresh_ethane + R_ethane); % ?? DELETE ME 
					F_steam = MR_MOL_2_MASS_CONV_FACTOR * MR_S_i * (F_fresh_ethane);
...
function cost = cost_separation_system_new(P_flowrates, F_steam, R_ethane, opt)
...
function cf = get_npv(npv)
	...
	npv.land = 0; % TJ / Chada told me to fix the land cost to zero
